...
type RecognitionLike = {
  ...
  onerror: ((event: RecognitionErrorLike) => void) | null; onend: (() => void) | null;
  start(): void; stop(): void; abort(): void;
};
type RecognitionConstructor = { new(): RecognitionLike; available?: (options: { langs: string[]; processLocally: boolean }) => Promise<string> };
...
function permissionDeniedReason(reason: string | undefined): boolean { return reason === "not-allowed" || reason === "service-not-allowed"; }

export class BrowserSpeechPort implements SpeechPort {
  private recognition: RecognitionLike | null = null;
  private locality: SpeechCapability["locality"] = "browser-service";
  private readonly errorListeners = new Set<(error: SpeechAccessError) => void>();
  ...
  setErrorListener(listener: (error: SpeechAccessError) => void): () => void {
  ...
  async capability(language: string): Promise<SpeechCapability> {
    ...
    } catch { /* Permissions.query is not uniformly implemented; recognition remains authoritative. */ }
    ...
    if (Recognition.available) {
      ...
      } catch { /* An inconclusive probe must never be reported as local processing. */ }
    }
    this.locality = "browser-service";
    return { available: true, locality: "browser-service", language, reason: "The browser may use its speech service to transcribe audio." };
  }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      recognition.onresult = (event) => {
        ...
        for (let index = event.resultIndex ?? 0; index < event.results.length; index += 1) {
          ...
          if (result?.isFinal) onFinal(transcript); else interim = `${interim} ${transcript}`.trim();
        }
      ...
      recognition.onerror = (event) => {
        ...
        else if (!(code === "aborted" && this.ignoreAbort)) for (const listener of this.errorListeners) listener(error);
        this.ignoreAbort = false;
      };
      recognition.onend = () => { this.recognition = null; this.ignoreAbort = false; };
      try { recognition.start(); }
      catch { this.recognition = null; reject(new SpeechAccessError("failed", "Speech recognition could not start.")); }
    });
  }

  stop(): void { this.recognition?.stop(); }
  cancel(): void { this.ignoreAbort = true; this.recognition?.abort(); this.recognition = null; }
}
