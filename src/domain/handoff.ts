...
export type HandoffSummary = { feeds: number; sleepSessions: number; diapers: number; sleepMinutes: number; openTimers: number; pumpingSessions: number; pumpingMinutes: number; solids: number; tummyTimeSessions: number; tummyTimeMinutes: number };
export function summarizeHandoffPayload(payload: HandoffPayload): HandoffSummary {
  ...
  if (checked.v === 3) return { feeds: checked.totals.feeds, sleepSessions: checked.totals.sleepSessions, diapers: checked.totals.diapers, sleepMinutes: checked.totals.sleepMinutes, openTimers: checked.openTimerCount, pumpingSessions: checked.totals.pumpingSessions, pumpingMinutes: checked.totals.pumpingMinutes, solids: checked.totals.solids, tummyTimeSessions: checked.totals.tummyTimeSessions, tummyTimeMinutes: checked.totals.tummyTimeMinutes };
  let sleepSessions = 0, sleepMinutes = 0; for (const event of checked.events) if (event.type === "sleep") { sleepSessions += 1; if (event.endedAt) sleepMinutes += Math.max(0, durationMinutes(event.at, event.endedAt)); }
  return { feeds: checked.events.filter((event) => event.type === "feed").length, sleepSessions, diapers: checked.events.filter((event) => event.type === "diaper").length, sleepMinutes: Math.round(sleepMinutes), openTimers: checked.openTimerCount, pumpingSessions: 0, pumpingMinutes: 0, solids: 0, tummyTimeSessions: 0, tummyTimeMinutes: 0 };
}
