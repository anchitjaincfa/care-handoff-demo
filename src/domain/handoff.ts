...
export const HandoffPayloadSchema = z.object({ v: z.literal(1), provenance: z.enum(["real", "demo"]), generatedAt: UtcInstantSchema, expiresAt: UtcInstantSchema, babyLabel: z.string().min(1).max(40), shiftStart: UtcInstantSchema, shiftEnd: UtcInstantSchema, events: z.array(HandoffEventProjectionSchema).max(30), openTimerCount: z.number().int().nonnegative() }).strict();
...
export function encodeHandoffFragment(payload: HandoffPayload, transport: HandoffTransport = "qr"): string {
...
export function generateHandoffPayload(input: { events: CareEvent[]; provenance: "real" | "demo"; generatedAt: string; babyLabel: string; shiftStart: string; shiftEnd: string }): HandoffPayload {
  ...
  return HandoffPayloadSchema.parse({ v: 1, provenance: input.provenance, generatedAt: input.generatedAt, expiresAt: addHours(input.generatedAt, HANDOFF_EXPIRY_HOURS), babyLabel: input.babyLabel, shiftStart: input.shiftStart, shiftEnd: input.shiftEnd, events, openTimerCount });
}
