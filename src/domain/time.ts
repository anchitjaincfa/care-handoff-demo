...
const CLOCK_24_HOUR = /\b(?:at\s+)?([01]\d|2[0-3]):([0-5]\d)\b/i;
...
export function addHours(instant: string, hours: number): string { return addMinutes(instant, hours * 60); }

export function zonedDateTimeToInstant(date: string, time: { hour: number; minute?: number; second?: number }, timeZone: string, disambiguation: "compatible" | "earlier" | "later" | "reject" = "compatible"): string {
  const plainDate = Temporal.PlainDate.from(date);
  return toUtcInstant(Temporal.ZonedDateTime.from({ timeZone: assertTimeZone(timeZone), year: plainDate.year, month: plainDate.month, day: plainDate.day, hour: time.hour, minute: time.minute ?? 0, second: time.second ?? 0, millisecond: 0 }, { disambiguation }).toInstant());
}
...
function clockParts(expression: string): { hour: number; minute: number } | Extract<TimeResolution, { ok: false }> | null {
  ...
  if (meridiem) {
    const rawHour = Number(meridiem[1]); if (rawHour < 1 || rawHour > 12) return { ok: false, reason: "invalid", explanation: "Clock hour is invalid." };
    const isPm = meridiem[3]?.toLowerCase().startsWith("p") ?? false; return { hour: (rawHour % 12) + (isPm ? 12 : 0), minute: Number(meridiem[2] ?? 0) };
  }
  const twentyFourHour = expression.match(CLOCK_24_HOUR); if (!twentyFourHour) return null;
  return { hour: Number(twentyFourHour[1]), minute: Number(twentyFourHour[2]) };
}
function isFailure(value: { hour: number; minute: number } | Extract<TimeResolution, { ok: false }>): value is Extract<TimeResolution, { ok: false }> { return "ok" in value && value.ok === false; }

export function resolvePastTime(expression: string, options: ResolveTimeOptions): TimeResolution {
  let now: Temporal.Instant;
  try { now = Temporal.Instant.from(options.now); assertTimeZone(options.timeZone); } catch { return { ok: false, reason: "invalid", explanation: "The reference time or time zone is invalid." }; }
  const normalized = expression.trim().toLowerCase();
  if (/\b(tomorrow|later|next\s+(?:hour|day|week)|in\s+\d+\s*(?:minutes?|hours?))\b/.test(normalized)) return { ok: false, reason: "future", explanation: "Future times are not logged as completed care events." };
  const relative = normalized.match(RELATIVE_PAST);
  if (relative) { const amount = Number(relative[1]); const minutes = relative[2]?.toLowerCase().startsWith("h") ? amount * 60 : amount; if (!Number.isFinite(minutes) || minutes < 0) return { ok: false, reason: "invalid", explanation: "The relative time is invalid." }; return { ok: true, instant: addMinutes(toUtcInstant(now), -minutes), kind: "relative" }; }
  if (/\bnow\b/.test(normalized)) return { ok: true, instant: toUtcInstant(now), kind: "now" };

  const parsedClock = clockParts(normalized); if (parsedClock && isFailure(parsedClock)) return parsedClock;
  if (parsedClock) {
    const currentLocal = now.toZonedDateTimeISO(options.timeZone); const explicitDateMatch = normalized.match(ISO_DATE); let date: Temporal.PlainDate;
    try { date = explicitDateMatch ? Temporal.PlainDate.from(explicitDateMatch[1] ?? "") : currentLocal.toPlainDate().subtract({ days: /\byesterday\b/.test(normalized) ? 1 : 0 }); } catch { return { ok: false, reason: "invalid", explanation: "The calendar date is invalid." }; }
    let candidate: Temporal.Instant;
    try { candidate = Temporal.Instant.from(zonedDateTimeToInstant(date.toString(), parsedClock, options.timeZone, "reject")); } catch { return { ok: false, reason: "ambiguous", explanation: "That local clock time is skipped or repeated in this time zone." }; }
  ...
  if (/\b(?:at\s+)?\d{1,2}(?::[0-5]\d)?\b/.test(normalized) && /\bat\s+\d|\b\d{1,2}:[0-5]\d\b/.test(normalized)) return { ok: false, reason: "ambiguous", explanation: "Use am or pm, or a two-digit 24-hour clock." };
