...
export type DemoSeedOptions = { householdId?: string; babyId?: string; timeZone?: string; startDate?: string; days?: number };

export function createDemoSeed(options: DemoSeedOptions = {}): CareEvent[] {
  const householdId = options.householdId ?? "demo-household"; const babyId = options.babyId ?? "demo-baby"; const timeZone = options.timeZone ?? "America/Los_Angeles"; const startDate = Temporal.PlainDate.from(options.startDate ?? "2026-07-20"); const days = options.days ?? 8; const events: CareEvent[] = [];
