  ...
  return (
    <>
      ...
      <div className="privacy-grid">
        <section className="privacy-card"><span className="privacy-card__icon"><Icon name="shield" /></span><div><h2>{COPY.privacy.storageTitle}</h2><p>{COPY.privacy.storageBody}</p><button className="button button--soft" type="button" onClick={() => void props.onRequestPersistence()} disabled={props.storage === "requesting" || props.storage === "granted"}>{storageLabel}</button></div></section>
        ...
        <section className="privacy-card"><span className="privacy-card__icon"><Icon name="arrow" /></span><div><h2>{COPY.privacy.importTitle}</h2><p>{COPY.privacy.importBody}</p><label className="file-button"><input type="file" accept=".json,application/json" onChange={(event) => void chooseFile(event.target.files?.[0])} disabled={props.importState.status === "reading" || props.importState.status === "importing"} />{COPY.live.chooseBackup}</label></div></section>
      ...
      {props.importState.status === "review" && <section className="brief-card" role="dialog" aria-modal="true" aria-labelledby="import-review-title"><h2 id="import-review-title">{COPY.live.importPreviewTitle}</h2><p>{props.importState.fileName}{COPY.live.separator}{props.importState.eventCount}</p>{props.importState.warnings.length > 0 && <div><h3>{COPY.live.importWarnings}</h3><ul>{props.importState.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></div>}<div className="button-row"><button className="button button--ghost" type="button" onClick={() => void props.onCancelImport()}>{COPY.global.cancel}</button><button className="button button--primary" type="button" onClick={() => void props.onConfirmImport()}>{COPY.live.importConfirm}</button></div></section>}
