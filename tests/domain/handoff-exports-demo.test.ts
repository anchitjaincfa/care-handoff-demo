...
describe("handoff generation and codec", () => {
  ...
  it("retains legacy envelope parsing but refuses v1 and v2 sharing", () => {
    ...
    const common = { provenance: current.provenance, generatedAt: current.generatedAt, expiresAt: current.expiresAt, babyLabel: current.babyLabel, shiftStart: current.shiftStart, shiftEnd: current.shiftEnd, events: [], openTimerCount: current.openTimerCount };
